...
@Component({
  selector: 'app-pose-group',
  templateUrl: './pose-group.component.html',
  styleUrls: ['./pose-group.component.css']
})
export class PoseGroupComponent implements OnInit {
  user_id: any;
  profile: any;
  token: { headers: HttpHeaders; };
  setUploadShowAndDrop = false;
  ...
  async selectPose() {
    ...
    let json = { user_id: this.user_id, pose_id: this.pose_max, working: this.working ,group_id: this.groupPage};

    console.log(json);
    this.http.post("http://203.154.83.62:1238/select/group/pose_all", JSON.stringify(json), this.token).subscribe(async response => {
