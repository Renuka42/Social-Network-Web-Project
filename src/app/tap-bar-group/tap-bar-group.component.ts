...
@Component({
  selector: 'app-tap-bar-group',
  templateUrl: './tap-bar-group.component.html',
  styleUrls: ['./tap-bar-group.component.css']
})
export class TapBarGroupComponent implements OnInit {
...
selectFriend() {
  let json = { user_id: this.user_id ,group_id:this.groupPage};
  this.http.post("http://203.154.83.62:1238/select/group/friend", JSON.stringify(json), this.token).subscribe(response => {
    var array = Object.values(response);
